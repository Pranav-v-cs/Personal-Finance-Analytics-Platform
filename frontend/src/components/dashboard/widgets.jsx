...
function getHealthScoreColor(label) {
  if (!label) return 'text-[var(--text)]'
  const key = label.toLowerCase().replace(/\s+/g, '-')
  return healthScoreColors[key] || 'text-[var(--text)]'
}

export const WidgetFinancialHealth = memo(function WidgetFinancialHealth({ health }) {
  return (
    <div className={cn('flex flex-col gap-5', health.score === null && 'opacity-60')}>
      {health.score !== null ? (
        <>
          <div className="flex flex-col items-center gap-1">
            <span className={cn('text-[clamp(2.5rem,5vw,3.5rem)] font-black leading-none tracking-tight', getHealthScoreColor(health.label))}>
              {health.score}
            </span>
            <span className="text-sm font-semibold text-[var(--muted)]">{health.label}</span>
            {health.scoreChange !== null && health.scoreChange !== 0 && (
              <Badge variant={health.scoreChange > 0 ? 'success' : 'danger'}>
                {health.scoreChange > 0 ? '▲' : '▼'} {Math.abs(health.scoreChange)} pts
              </Badge>
            )}
          </div>
          <div className="flex flex-col gap-1.5">
            <p className="text-sm text-[var(--muted)] leading-relaxed">{health.recommendation}</p>
            {health.actionRecommendation && (
              <p className="text-sm font-medium text-[var(--accent)]">{health.actionRecommendation}</p>
            )}
          </div>
        </>
      ) : (
        <>
          <div className="flex flex-col items-center gap-1">
            <span className="text-[clamp(2.5rem,5vw,3.5rem)] font-black leading-none tracking-tight text-gray-400">--</span>
            <span className="text-sm font-semibold text-[var(--muted)]">Pending</span>
          </div>
          <div className="flex flex-col gap-1.5">
            <p className="text-sm text-[var(--muted)] leading-relaxed">{health.recommendation}</p>
            {health.actionRecommendation && (
      ...
      {health.metrics && (
        <div className="flex flex-col gap-3">
          <div className="flex flex-col gap-1.5">
            <span className="text-xs font-medium text-[var(--muted)]">Spending</span>
            <div className="h-2 w-full overflow-hidden rounded-full bg-[rgba(255,255,255,0.06)]">
              <div className="h-full rounded-full transition-all duration-300" style={{ width: `${health.metrics.spendingScore}%`, background: health.metrics.spendingScore >= 70 ? '#66bb6a' : health.metrics.spendingScore >= 50 ? '#f7b14a' : '#ef5350' }} />
            </div>
          </div>
          {health.metrics.budgetScore !== null && (
            <div className="flex flex-col gap-1.5">
              ...
              <div className="h-2 w-full overflow-hidden rounded-full bg-[rgba(255,255,255,0.06)]">
                <div className="h-full rounded-full transition-all duration-300" style={{ width: `${health.metrics.budgetScore}%`, background: health.metrics.budgetScore >= 70 ? '#66bb6a' : health.metrics.budgetScore >= 50 ? '#f7b14a' : '#ef5350' }} />
              </div>
            </div>
          )}
          {health.metrics.goalScore !== null && (
            <div className="flex flex-col gap-1.5">
              <span className="text-xs font-medium text-[var(--muted)]">Goals</span>
              <div className="h-2 w-full overflow-hidden rounded-full bg-[rgba(255,255,255,0.06)]">
                <div className="h-full rounded-full transition-all duration-300" style={{ width: `${health.metrics.goalScore}%`, background: health.metrics.goalScore >= 70 ? '#66bb6a' : health.metrics.goalScore >= 50 ? '#f7b14a' : '#ef5350' }} />
              </div>
            </div>
          )}
        </div>
